...
| **AUTH-TC-005** | Đăng ký tài khoản | **Positive – Driver đăng ký tài khoản với thông tin hợp lệ** | Driver chưa có tài khoản và đang ở chức năng đăng ký | 1. Mở chức năng đăng ký<br>2. Nhập đầy đủ thông tin Driver hợp lệ<br>3. Gửi yêu cầu | Actor = Driver<br>Dữ liệu đăng ký = valid | Hệ thống tạo tài khoản Driver thành công | High |
| **AUTH-TC-006** | Đăng ký tài khoản Driver | **Positive – Operation Staff tạo tài khoản Driver theo quy trình quản lý tài xế** | Operation Staff đã đăng nhập và có quyền quản lý Driver | 1. Mở chức năng Quản lý tài xế<br>2. Chọn tạo tài khoản Driver<br>3. Nhập thông tin hợp lệ<br>4. Lưu | Actor = Operation Staff<br>Driver data = valid | Hệ thống tạo tài khoản Driver thành công | High |
...
| **TRIP-TC-019** | Theo dõi chuyến | **Positive – Customer xem trạng thái hiện tại và thông tin Driver đã nhận chuyến** | Customer đã đăng nhập và có Trip đang thực hiện | 1. Mở chức năng Theo dõi chuyến<br>2. Xem trạng thái hiện tại<br>3. Kiểm tra thông tin Driver đã nhận chuyến | Trip = active<br>Assigned Driver = available | Hệ thống hiển thị trạng thái hiện tại của chuyến và thông tin Driver đã nhận chuyến cho Customer | High |
| **TRIP-TC-020** | Hoàn thành chuyến | **Boundary – Sau khi Trip chuyển sang Completed thì bắt đầu bước tính cước** | Trip đang ở trạng thái trước Completed | 1. Kiểm tra trước khi hoàn thành<br>2. Cập nhật Trip = Completed<br>3. Kiểm tra bước tiếp theo | Trip status: active → Completed | Trước Completed chưa tính cước; sau Completed hệ thống chuyển sang xác định số tiền phải trả | High |
 ...
 | Test Case ID | Test Scenario | Test Case | Preconditions | Test Steps | Test Data | Expected Result | Priority |
|---|---|---|---|---|---|---|---|
| **FP-TC-001** | Xác định số tiền phải trả | **Positive – Xác định số tiền phải trả sau khi chuyến đã hoàn thành** | Trip tồn tại và đã ở trạng thái Completed | 1. Hoàn thành Trip<br>2. Yêu cầu hệ thống xác định số tiền phải trả<br>3. Kiểm tra kết quả | Trip status = Completed<br>Trip information = valid | Hệ thống xác định số tiền khách hàng phải trả dựa trên loại dịch vụ và thông tin chuyến đi | High |
| **FP-TC-002** | Xác định số tiền phải trả | **Boundary – Không xác định số tiền phải trả trước khi Trip chuyển sang Completed** | Trip đang được thực hiện và chưa hoàn thành | 1. Giữ Trip ở trạng thái chưa Completed<br>2. Yêu cầu hệ thống xác định số tiền phải trả | Trip status ≠ Completed | Hệ thống chưa thực hiện bước xác định số tiền phải trả khi chuyến chưa hoàn thành | High |
| **FP-TC-003** | Xác định số tiền phải trả | **Boundary – Xác định số tiền phải trả ngay sau khi Trip chuyển sang Completed** | Trip đang ở trạng thái trước Completed | 1. Cập nhật Trip sang Completed<br>2. Kiểm tra bước xử lý tiếp theo | Trip status: active → Completed | Sau khi Trip hoàn thành, hệ thống chuyển sang bước xác định số tiền khách hàng phải trả | High |
| **FP-TC-004** | Xác định số tiền phải trả | **Negative – Trip không tồn tại** | Hệ thống đang hoạt động | 1. Gửi yêu cầu xác định số tiền phải trả cho Trip không tồn tại | Trip ID = not found | Hệ thống không xác định số tiền phải trả cho Trip không tồn tại và trả kết quả phù hợp | Medium |
| **FP-TC-005** | Xác định số tiền phải trả | **Empty/Null – Không truyền Trip ID** | Fare API đang hoạt động | 1. Gửi yêu cầu xác định số tiền phải trả<br>2. Không truyền Trip ID | Trip ID = null | Hệ thống từ chối request do thiếu thông tin xác định chuyến | High |
| **FP-TC-006** | Xác định số tiền phải trả | **Invalid Format/Value – Trip ID sai định dạng** | Fare API có quy định định dạng Trip ID | 1. Gửi request với Trip ID sai định dạng | Trip ID = invalid format | Hệ thống từ chối giá trị Trip ID không hợp lệ | Medium |
